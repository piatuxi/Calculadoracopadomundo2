...
const ESPN_SCOREBOARD_URL = "https://site.api.espn.com/apis/site/v2/sports/soccer/fifa.world/scoreboard?dates=20260611-20260627&limit=200";
...
function scoreStatusFromEspn(status = {}) {
  const type = status.type || {};
  if (type.completed || type.state === "post") return "done";
  if (type.state === "in") return "live";
  return "future";
}

function mergeEspnScoreboard(schedule, scoreboard) {
  const events = Array.isArray(scoreboard?.events) ? scoreboard.events : [];
  if (!events.length) return { schedule, liveScores: [] };

  const byTeams = new Map(schedule.map((row) => [`${row[2]}|${row[3]}`, row]));
  const liveScores = [];
  ...
  for (const event of events) {
    ...
    const home = normalizeSourceName(homeEntry.team?.displayName || homeEntry.team?.name || "");
    const away = normalizeSourceName(awayEntry.team?.displayName || awayEntry.team?.name || "");
    const key = `${home}|${away}`;
    const row = byTeams.get(key);
    if (!row) continue;

    const homeGoals = Number(homeEntry.score || 0);
    const awayGoals = Number(awayEntry.score || 0);
    const status = scoreStatusFromEspn(competition.status || event.status || {});
    ...
    liveScores.push({ group: row[0], date: row[1], home, away, homeGoals, awayGoals, status, detail, eventId: event.id, kickoff: event.date || competition.date || "" });
  }

  return { schedule, liveScores };
}
...
function standingsFromSchedule(schedule) {
  ...
  for (const row of schedule) {
    const [, , home, away, homeGoals, awayGoals, meta] = row;
    if (meta?.status && meta.status !== "done") continue;
...
async function buildSnapshot() {
  const [scheduleHtml, fixturesText, espnScoreboard] = await Promise.all([
    fetchText(SCHEDULE_URL),
    fetchText(ELO_FIXTURES_URL),
    fetchJson(ESPN_SCOREBOARD_URL).catch(() => null)
  ]);
  const parsedSchedule = parseScheduleFromArticle(scheduleHtml);
  const { schedule, liveScores } = mergeEspnScoreboard(parsedSchedule, espnScoreboard);
  ...
  return {
    ...
    sources: {
      ...
      liveScores: ESPN_SCOREBOARD_URL
    }
  };
}
...
function premiumEmailFromPayment(payment) {
  const fromReference = String(payment.external_reference || "").replace("premium:", "");
  return [
...
app.post("/api/premium/create-pix", async (req, res) => {
  ...
  try {
    ...
    console.log("premium pix created", { paymentId: payment.id, status: payment.status, email });
...
app.post("/api/premium/mercadopago/webhook", async (req, res) => {
  ...
  console.log("premium webhook received", {
    action: req.body?.action || req.body?.type || "",
    paymentId: paymentId || "",
    query: req.query || {}
  });
  if (!paymentId) {
    console.log("premium webhook ignored: missing payment id");
    return;
  }

  try {
    const payment = await fetchMercadoPagoPayment(paymentId);
    console.log("premium payment fetched", {
      paymentId,
      status: payment.status,
      externalReference: payment.external_reference || ""
    });
    if (payment.status !== "approved") {
      console.log("premium webhook ignored: payment not approved", { paymentId, status: payment.status });
      return;
    }
    const email = premiumEmailFromPayment(payment);
    if (!email || !email.includes("@")) {
      console.log("premium webhook ignored: missing payer email", { paymentId });
      return;
    }
    await sendPremiumEmail(email);
    console.log("premium email sent", { paymentId, email });
  } catch (error) {
